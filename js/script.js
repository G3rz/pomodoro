...
const tomatoButton = document.getElementById('start'),
...
function start() {
    ...
    interval = setInterval(function(){
        ...
        if (1200 > number && 'Start'!= title.textContent) {
            greenBackground();
        }

        if (1200 == number) {
            beep();
            favicon.href = "favicon/stop.ico"
...
tomatoButton.addEventListener('click',function(){
    tomatoButton.classList.toggle('start');
    if ('start'== tomatoButton.classList){
        start();
    } else {
        stop();
    }
